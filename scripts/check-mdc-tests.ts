...
readdirSync(join(srcDirectory, 'material-experimental'), {withFileTypes: true})
    .reduce(
        (matches, entity) => {
          // Go through all the `material-experimental` directories and match them to ones in
          // `material`.
    ...
    .forEach((mdcPackage, materialPackage) => {
      ...
      if (mdcTestFiles.length > 0) {
        ...
        const missingTests =
            materialTests.filter(test => !mdcTests.includes(test) && !skippedTests.includes(test));
...
function getUnitTestFiles(name: string): string[] {
  return glob('{,!(testing)/**/}!(*.e2e).spec.ts', {absolute: true, cwd: join(srcDirectory, name)});
}
  ...
  files.forEach(file => {
    ...
    sourceFile.forEachChild(function walk(node: ts.Node) {
      if (ts.isCallExpression(node) && ts.isIdentifier(node.expression) &&
        ...
        testNames.push(node.arguments[0]
                           .getText(sourceFile)
