...
export class TemporalDateAdapter extends DateAdapter<TemporalDateType> {
  ...
  getMonthNames(style: 'long' | 'short' | 'narrow'): string[] {
    const monthsInYear = this._getMonthsInYearForDate(2017);
  ...
  getDateNames(): string[] {
    ...
    return range(31, i => {
      const date = Temporal.PlainDate.from({
        ...
        month: 1,
        day: i + 1,
        calendar: 'iso8601', // Use ISO calendar for date names
      });
      return this._formatWithLocale(date, options);
    });
  }
  ...
  toIso8601(date: TemporalDateType): string {
    if (this._isZonedDateTime(date)) {
      ...
      return this._toZonedDateTime(date).toString();
    }
    if (this._isPlainDateTime(date)) {
  ...
  override setTime(
    ...
    if (this._isZonedDateTime(target)) {
      return this._toZonedDateTime(target).with({
        hour: hours,
        minute: minutes,
        second: seconds,
        millisecond: 0,
    ...
    if (this._mode === 'zoned') {
      return this._toPlainDate(target)
        ...
        .toZonedDateTime(this._getTimeZoneId());
    }
  ...
  override addSeconds(date: TemporalDateType, amount: number): TemporalDateType {
    ...
    if (this._mode === 'zoned') {
      return dateTime.toZonedDateTime(this._getTimeZoneId()).add({seconds: amount});
    }
  ...
  private _createFromEpochMs(ms: number): TemporalDateType {
    ...
    if (this._mode === 'zoned') {
      return instant.toZonedDateTimeISO(this._getTimeZoneId()).withCalendar(this._getCalendarId());
    }
