...
class TestItemComponent implements HasElement, OnDestroy {
  readonly element = inject(ElementRef).nativeElement as HTMLElement;
  private readonly _parent = inject(TestCollectionComponent);

  @Input() value!: string;

  constructor() {
    afterNextRender(() => {
      this._parent._collection.register(this);
    });
  }

  ngOnDestroy() {
    this._parent._collection.unregister(this);
  }
}
