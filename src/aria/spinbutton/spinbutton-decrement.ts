...
/** Default interval between auto-repeat decrements (in ms). */
const DEFAULT_AUTO_REPEAT_INTERVAL = 60;

/**
 * A decrement button for a spinbutton.
 *
 * Must be used within an `ngSpinButtonGroup`. Automatically binds to the
 * spinbutton in the group and handles disabled state when at minimum value.
...
export class SpinButtonDecrement implements OnDestroy {
  ...
  readonly _pattern: SpinButtonStepperPattern = new SpinButtonStepperPattern({
    ...
    doOperation: () => this._group.spinButton()?.doDecrement(),
