import {Component, DebugElement, computed, model, signal, viewChildren} from '@angular/core';
  ...
  template: `
    <div>
      <ul ngTabList
        ...
        @for (tabDef of tabsData(); track tabDef.label; let i = $index) {
          <li ngTab [panel]="tabPanels()[i]" [disabled]="!!tabDef.disabled">{{ tabDef.label }}</li>
        }
      </ul>
...
class TestTabsComponent {
  ...
  tabPatterns = computed(() => this.tabs().map(tab => tab._pattern));
}
