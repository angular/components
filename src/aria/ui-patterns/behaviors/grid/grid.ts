...
export class Grid<T extends GridCell> {
  ...
  /** Resets the focus state of the grid if it is empty or stale. */
  resetState(): boolean {
    ...
    if (this.focusBehavior.stateStale()) {
      return (
        this.focusBehavior.focusCell(this.focusBehavior.activeCell()!) ||
        this.focusBehavior.focusCoordinates(this.focusBehavior.activeCoords())
      );
    }
