...
export class GridCellPattern implements GridCell {
  ...
  readonly pauseGridNavigation = computed(
    () => this.inputs.widget()?.pauseGridNavigation() ?? false,
  );
  ...
  /** Resets the navigation state of the widget within the cell, allowing grid navigation to resume. */
  resetGridNavigation(): void {
    this.inputs.widget()?.pauseGridNavigation.set(false);
  }
}
