...
import {DEFAULT_PLAYER_HEIGHT, DEFAULT_PLAYER_WIDTH, YouTubePlayer} from './youtube-player';

const VIDEO_ID = 'a12345';
const YT_LOADING_STATE_MOCK = {
  loading: 1,
  loaded: 0
};
...
describe('YoutubePlayer', () => {
  ...
  describe('API ready', () => {
    ...
    it('initializes a youtube player', () => {
      ...
      expect(playerCtorSpy).toHaveBeenCalledWith(containerElement, jasmine.objectContaining({
        videoId: VIDEO_ID,
    ...
    it('responds to changes in video id', () => {
      ...
      expect(playerSpy.cueVideoById).toHaveBeenCalledWith(jasmine.objectContaining({
        videoId: 'otherId'
      }));
    ...
    it('initializes the player with start and end seconds', () => {
      ...
      expect(playerSpy.cueVideoById)
          .toHaveBeenCalledWith(jasmine.objectContaining({startSeconds: 5, endSeconds: 6}));

      testComponent.endSeconds = 8;
      fixture.detectChanges();

      expect(playerSpy.cueVideoById)
          .toHaveBeenCalledWith(jasmine.objectContaining({startSeconds: 5, endSeconds: 8}));

      testComponent.startSeconds = 7;
      fixture.detectChanges();

      expect(playerSpy.cueVideoById)
          .toHaveBeenCalledWith(jasmine.objectContaining({startSeconds: 7, endSeconds: 8}));
      ...
      expect(playerSpy.cueVideoById)
          .toHaveBeenCalledWith(jasmine.objectContaining({startSeconds: 10, endSeconds: 11}));
    });
  ...
  it('should pick up static startSeconds and endSeconds values', () => {
    ...
    expect(playerSpy.cueVideoById)
        .toHaveBeenCalledWith(jasmine.objectContaining({startSeconds: 42, endSeconds: 1337}));
  });
...
@Component({template: `<youtube-player [videoId]="videoId"></youtube-player>`})
class NoEventsApp {
