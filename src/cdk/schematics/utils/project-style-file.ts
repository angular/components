...
export function getProjectStyleFile(project: ProjectDefinition, extension?: string): string|null {
  ...
  if (buildOptions.styles && isJsonArray(buildOptions.styles) && buildOptions.styles.length) {
    ...
    const defaultMainStylePath = styles.find(
        file => extension ? file === `styles.${extension}` : defaultStyleFileRegex.test(file));
    ...
    // file extension.
    const fallbackStylePath = styles.find(
        file => extension ? file.endsWith(`.${extension}`) : validStyleFileRegex.test(file));
