  ...
  private _visitSourceFile(sourceFile: ts.SourceFile) {
    ...
    for (const diagnostic of diagnostics) {
      ...
      const signatures = classSignatures
        .map(signature => signature.map(t => this.typeChecker.typeToString(t)))
...
function getParameterTypesFromSignature(signature: ts.Signature,
                                        typeChecker: ts.TypeChecker): ts.Type[] {
  return signature.getParameters()
    .map(param => typeChecker.getTypeAtLocation(param.declarations[0]));
}
