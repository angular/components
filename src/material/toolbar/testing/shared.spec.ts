...
import {
  MatToolbarHarness,
  MatToolbarSection
} from '@angular/material/toolbar/testing/toolbar-harness';
import {ComponentFixture, TestBed} from '@angular/core/testing';
import {TestbedHarnessEnvironment} from '@angular/cdk/testing/testbed';

export function runHarnessTexts(
  ...
  it('should find toolbar with text', async () => {
    ...
    expect(await toolbars[0].isMultiRow()).toBeFalse();
  })
  ...
  it('should find toolbar with regex', async () => {
    ...
    expect(toolbars.length).toBe(1);
    expect(await toolbars[0].isMultiRow()).toBeTrue();
  })

  it('should get toolbar text', async () => {
    const toolbars = await loader.getAllHarnesses(toolbarHarness);

    expect(await Promise.all(toolbars.map(t => t.getText()))).toEqual([
      'My App',
      'Row 1Row 2 Button 1  Button 2'
    ])
  })

  it('should get harness loaders for toolbar row', async () => {
    const toolbar = await loader.getHarness(toolbarHarness.with({text: /Button/}));
    const rowLoaders = await toolbar.getAllChildLoaders(MatToolbarSection.INNER_ROW);
    const row1 = await rowLoaders[0] as HarnessLoader;
    const row1Subcomponents = await row1.getAllHarnesses(DummyHarness) ?? [];
    const row2 = await rowLoaders[1] as HarnessLoader;
