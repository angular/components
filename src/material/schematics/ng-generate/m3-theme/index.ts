...
/**
 * Gets map of all the color tonal palettes from a material dynamic scheme.
 * @param scheme Dynamic scheme that represents the theme.
 * @returns Map with the colors and their hue tones and values.
 */
function getColorPalettesMapFromDynamicScheme(scheme: DynamicScheme) {
  const tonalPalettes = {
 ...
 * @param systemVariablePrefix Prefix for system-level variables if applicable.
...
export function generateSCSSTheme(
  ...
  if (useSystemVariables && systemVariablePrefix) {
    scss = scss.concat(['$sys-var-prefix: ' + systemVariablePrefix + ';', '']);
  }
      ...
      ...(useSystemVariables
        ? [
            ...
            ...(systemVariablePrefix
              ? ['    system-variables-prefix: ' + systemVariablePrefix + ',']
              : []),
          ]
        : []),
      '  ),',
...
function generateHighContrastOverrideMixinsSCSS(
  ...
  for (const themeType of themeTypes) {
    ...
    scss += "  $sys-var-prefix: if(variable-exists(sys-var-prefix), $sys-var-prefix, 'sys');\n";
    for (const [key, value] of overrides!.entries()) {
      scss += "  #{'--' + $sys-var-prefix + '-" + key + "'}: " + value + ';\n';
    }
    scss += '};\n';
  }
  return scss;
...
export default function (options: Schema): Rule {
  return async (tree: Tree, context: SchematicContext) => {
    ...
    if (options.generateHighContrastOverrideMixins && !options.useSystemVariables) {
      context.logger.info(
        'High contrast theme mixins require using system variables to work. Enabling system variables for generated theme.',
      );
      options.useSystemVariables = true;
    }

    // Generate standard contrast theme
    let themeTypes = options.themeTypes === 'both' ? ['light', 'dark'] : [options.themeTypes];
    const scheme = getMaterialDynamicScheme(
      primaryPalette,
      secondaryPalette,
      tertiaryPalette,
    ...
    let themeScss = generateSCSSTheme(
      getColorPalettesMapFromDynamicScheme(scheme),
      themeTypes,
      colorComment,
      options.useSystemVariables || false,
      options.systemVariablePrefix,
    );

    // Add high contrast overrides mixin if specified
    if (options.generateHighContrastOverrideMixins) {
