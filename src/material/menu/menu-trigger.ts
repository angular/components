...
export class MatMenuTrigger implements AfterContentInit, OnDestroy {
  ...
  constructor(
      private _overlay: Overlay, private _element: ElementRef<HTMLElement>,
  ...
  private _destroyMenu(reason: MenuCloseReason) {
    ...
    if (menu instanceof _MatMenuBase) {
      ...
      if (menu.lazyContent) {
        ...
        menu._animationDone
            .pipe(
                filter(event => event.toState === 'void'), take(1),
  ...
  private _setPosition(positionStrategy: FlexibleConnectedPositionStrategy) {
    ...
    positionStrategy.withPositions([
      ...
      {originX, originY: originFallbackY, overlayX, overlayY: overlayFallbackY, offsetY: -offsetY},
      {
  ...
  private _menuClosingActions() {
    ...
    const hover = this._parentMaterialMenu ?
        this._parentMaterialMenu._hovered().pipe(
            filter(active => active !== this._menuItemInstance), filter(() => this._menuOpen)) :
        observableOf();
