...
export async function loadExampleFactory(
  ...
  const {moduleExports, moduleFactoryExports} =
      await loadModuleWithFactory(`@angular/components-examples/${module.importSpecifier}`);
...
async function loadModuleWithFactory(moduleName: string) {
  const [moduleFactoryExports, moduleExports] =
      await Promise.all([import(moduleName + '/index.ngfactory'), import(moduleName)]);
