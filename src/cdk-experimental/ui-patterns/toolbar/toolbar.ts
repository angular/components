...
export class ToolbarPattern<V> {
  ...
  private _next() {
    ...
    if (item instanceof ToolbarWidgetGroupPattern) {
      const {leaveGroup} = item.actions().next(false) ?? {};
  ...
  private _prev() {
    const item = this.inputs.activeItem();
    if (item instanceof ToolbarWidgetGroupPattern) {
      const {leaveGroup} = item.actions().prev(false) ?? {};
      if (!leaveGroup) return;
    }
  ...
  private _groupNext() {
    ...
    item?.actions().next(true);
  }

  private _groupPrev() {
    const item = this.inputs.activeItem();
    if (item instanceof ToolbarWidgetPattern) return;
    item?.actions().prev(true);
  }
  ...
  setDefaultState() {
    ...
    if (firstItem instanceof ToolbarWidgetGroupPattern) {
      firstItem.actions().setDefaultState();
    }
  }
