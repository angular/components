...
@Directive({
  ...
  host: {
    ...
    '(keydown)': 'interaction.onKeydown($event)',
...
export class CdkToolbar<V> {
  ...
  /**
   * The interaction manager for the toolbar
   * which translates DOM events into toolbar instructions.
   */
  readonly interaction: ToolbarInteraction = new ToolbarInteraction({
    orientation: this.orientation,
    textDirection: this.textDirection,
    handler: () => (i => this.pattern.execute(i)) as ToolbarInstructionHandler,
  });
...
export class CdkToolbarWidgetGroup<V> implements OnInit, OnDestroy {
  ...
  /** The handler for instructions that are passed to the widget group. */
  readonly handler = signal<ToolbarWidgetGroupInteractionHandler<V>>(() => {});

  /** The ToolbarWidgetGroup UIPattern. */
  pattern = new ToolbarWidgetGroupPattern<V>({
    ...this,
    id: this.id,
    element: this.element,
  });
