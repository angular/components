...
describe('MDC-based MatSlideToggle with forms', () => {
  ...
  describe('using ngModel', () => {
    ...
    it('should not throw an error when disabling while focused', fakeAsync(() => {
         expect(() => {
           // Focus the button element because after disabling, the `blur` event should
           // automatically fire and not result in a changed after checked exception. Related:
           // #12323
           buttonElement.focus();
           tick();
...
@Component({template: `<mat-slide-toggle checked disabled>Label</mat-slide-toggle>`})
