...
var doc$$inline_7 = goog.global.document;
goog.userAgent.DOCUMENT_MODE = doc$$inline_7 && goog.userAgent.IE ? goog.userAgent.getDocumentMode_() || ("CSS1Compat" == doc$$inline_7.compatMode ? parseInt(goog.userAgent.VERSION, 10) : 5) : void 0;
...
bcx.polyjuice.Client = function() {
  this.countries_ = this.lastError_ = null;
};
...
bcx.polyjuice.Client.prototype.load = function(cb) {
  ...
  this.lastError_ ? cb(bcx.promisechrome.error(this.lastError_)) : bcx.promisechrome.getCookie({url:bcx.polyjuice.consts.SSO_URL, name:bcx.polyjuice.consts.SSO_COOKIE_NAME}).then(goog.bind(this.cachedCountriesOrFetch_, this, cb), goog.bind(this.authBeforeFetchCountries_, this, cb));
};
bcx.polyjuice.Client.prototype.authBeforeFetchCountries_ = function(cb) {
  bcx.polyjuice.Client.log_("openTabForPolyjuiceAuth_", "");
  var authUrl = bcx.polyjuice.consts.APPENGINE_URL + bcx.polyjuice.consts.AUTH_ROUTE;
...
bcx.polyjuice.Client.prototype.afterWebRequestCompleted_ = function(cb, details) {
  ...
  bcx.promisechrome.removeTab(details.tabId).then(goog.partial(bcx.promisechrome.getCookie, {url:bcx.polyjuice.consts.SSO_URL, name:bcx.polyjuice.consts.SSO_COOKIE_NAME})).then(goog.bind(this.fetchCountriesFromAppengine_, this)).then(function(resp) {
    cb(bcx.promisechrome.success(resp));
  }).thenCatch(function(err) {
    cb(bcx.promisechrome.error(JSON.stringify(err)));
  });
};
bcx.polyjuice.Client.prototype.cachedCountriesOrFetch_ = function(cb) {
  bcx.polyjuice.Client.log_("cachedCountriesOrFetch_", "");
  this.countries_ ? cb(bcx.promisechrome.success({countries:this.countries_})) : this.fetchCountriesFromAppengine_().then(function(resp) {
    cb(bcx.promisechrome.success(resp));
  }).thenCatch(goog.bind(this.onError_, this, cb));
};
bcx.polyjuice.Client.prototype.fetchCountriesFromAppengine_ = function() {
  bcx.polyjuice.Client.log_("fetchCountriesFromAppengine_", "");
  return bcx.promisexhrio.send(bcx.polyjuice.consts.APPENGINE_URL + bcx.polyjuice.consts.LIST_COUNTRIES_ROUTE, "GET", void 0, void 0, 0, !0, 200, !0, bcx.polyjuice.consts.XSSI_PREFIX).then(goog.bind(this.handleFetchCountriesResponse_, this));
};
bcx.polyjuice.Client.prototype.handleFetchCountriesResponse_ = function(resp) {
  bcx.polyjuice.Client.log_("handleFetchCountriesResponse_", "");
  var data = resp.json;
  if (!data || !data.country) {
    return goog.Promise.reject("Server returned no data");
  }
  this.countries_ = data.country;
  return goog.Promise.resolve({countries:this.countries_});
};
bcx.polyjuice.Client.prototype.createSession = function(country) {
  bcx.polyjuice.Client.log_("createSession", "");
  return bcx.promisexhrio.send(bcx.polyjuice.consts.APPENGINE_URL + bcx.polyjuice.consts.CREATE_SESSION_ROUTE, "POST", "criteria=" + JSON.stringify({socketType:["TCP"], location:{country:country}}), void 0, 0, !0, 200, !0, bcx.polyjuice.consts.XSSI_PREFIX).then(bcx.polyjuice.validateCreateSessionResponse_).then(bcx.polyjuice.onCreateSessionSuccess_).thenCatch(goog.bind(this.onError_, this, void 0));
};
bcx.polyjuice.validateCreateSessionResponse_ = function(data) {
  ...
  var json = data.json;
  return json.sessionKey && json.egressPoint ? goog.Promise.resolve(json) : goog.Promise.reject("Server response has wrong keys.");
};
bcx.polyjuice.onCreateSessionSuccess_ = function(data) {
  bcx.polyjuice.Client.log_("onCreateSessionSuccess_", "");
  return bcx.promisechrome.sendMessage({command:bcx.polyjuice.consts.BEGIN_SESSION, sessionKey:data.sessionKey, egressPoint:data.egressPoint}, bcx.polyjuice.consts.WINKY_APP_ID).then(bcx.polyjuice.onSessionKeyMessageResponse_);
};
...
bcx.polyjuice.Client.prototype.endSession = function() {
  ...
  this.countries_ = this.lastError_ = null;
  return bcx.promisechrome.sendMessage({command:bcx.polyjuice.consts.END_SESSION}, bcx.polyjuice.consts.WINKY_APP_ID).then(bcx.polyjuice.onEndSessionResponse_).thenCatch(function(msg) {
    bcx.polyjuice.Client.log_("endSession", JSON.stringify(msg));
    return goog.Promise.resolve();
  });
};
bcx.polyjuice.onEndSessionResponse_ = function(response) {
  return bcx.promisexhrio.send(bcx.polyjuice.consts.APPENGINE_URL + bcx.polyjuice.consts.EXPIRE_SESSION_ROUTE, "POST", "sessionKey=" + encodeURIComponent(response.sessionKey), void 0, 0, !0, 200, !0, bcx.polyjuice.consts.XSSI_PREFIX).then(function(resp) {
...
bcx.Proxy.prototype.constructPacUrl_ = function(enabled) {
  ...
  enabled == bcx.consts.CHINA ? pac += bcx.Proxy.PAC_EXTRA_CHINA_ : enabled == bcx.consts.POLYJUICE && (pac += bcx.Proxy.PAC_EXTRA_POLYJUICE_);
...
bcx.Proxy.prototype.fetchBakedInPac_ = function() {
  ...
  var u = chrome.extension.getURL("beyondcorp_bundled.pac.js"), setPac = goog.bind(this.setBakedInPac_, this);
...
bcx.Client.prototype.pingCaptivePortalTestUrl_ = function() {
  ...
  bcx.promisexhrio.send(bcx.Client.CAPTIVE_PORTAL_URL_, "GET", "", {}, 1E3, !1, 204).then(function() {
    ...
    return goog.Timer.promise(1E3).then(ping);
  });
};
...
bcx.Client.prototype.onMessage_ = function(request, sender, cb) {
  ...
  switch(request.action) {
    ...
    case bcx.consts.SET_SETTINGS:
      return this.storage_.set(request.settings), this.proxy_.setProxy(this.proxy_.enabled()), cb(bcx.promisechrome.success()), !1;
    case bcx.consts.GET_POLYJUICE_COUNTRIES:
      return this.polyjuice_.load(cb), !0;
    case bcx.consts.START_POLYJUICE:
      return this.polyjuice_.createSession(request.country).then(function(resp) {
        cb(bcx.promisechrome.success(resp));
      }, function(err) {
        cb(bcx.promisechrome.error(JSON.stringify(err)));
      }), !0;
...
var s = new bcx.Storage, p = new bcx.polyjuice.Client;
