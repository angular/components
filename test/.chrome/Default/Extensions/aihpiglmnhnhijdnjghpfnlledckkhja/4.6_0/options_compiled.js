...
goog.userAgent.DOCUMENT_MODE = doc$$inline_3 && goog.userAgent.IE ? goog.userAgent.getDocumentMode_() || ("CSS1Compat" == doc$$inline_3.compatMode ? parseInt(goog.userAgent.VERSION, 10) : 5) : void 0;
...
goog.ui.Zippy.prototype.enableKeyboardEventsHandling_ = function(header) {
  header && this.keyboardEventHandler_.listen(header, goog.events.EventType.KEYDOWN, this.onHeaderKeyDown_);
};
...
goog.ui.Zippy.prototype.onHeaderKeyDown_ = function(event) {
  if (event.keyCode == goog.events.KeyCodes.ENTER || event.keyCode == goog.events.KeyCodes.SPACE) {
    this.toggle(), this.dispatchActionEvent_(), event.preventDefault(), event.stopPropagation();
  }
};
...
goog.ui.AnimatedZippy = function(header, content, opt_expanded, opt_domHelper) {
  var domHelper = opt_domHelper || goog.dom.getDomHelper(), elWrapper = domHelper.createDom(goog.dom.TagName.DIV, {style:"overflow:hidden"}), elContent = domHelper.getElement(content);
...
goog.ui.AnimatedZippy.prototype.setExpanded = function(expanded) {
  if (this.expanded_ != expanded || this.anim_) {
    ...
    var h = this.elContent_.offsetHeight, startH;
    this.anim_ ? (expanded = this.expanded_, goog.events.removeAll(this.anim_), this.anim_.stop(!1), startH = h - Math.abs(parseInt(this.elContent_.style.marginTop, 10))) : startH = expanded ? 0 : h;
    this.updateHeaderClassName(expanded);
...
bcx.Options = function() {
  ...
  this.textFields_ = this.checkboxes_ = null;
};
...
bcx.Options.prototype.saveOptions_ = function(e$$0) {
  bcx.Options.log_("saveOptions_", "id: " + e$$0.target.id);
